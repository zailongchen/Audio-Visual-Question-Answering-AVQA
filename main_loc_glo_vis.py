...
from dataloader.dataloader_avst_mimn_test import *
from models.net_HAVF import AVQA_Fusion_Net
import ast
import json
import numpy as np
import time
...
def train(args, model, train_loader, optimizer, criterion_qa, criterion_contr, criterion_tri, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        #  glo_audio_posi_feat, glo_visual_posi_feat, glo_visual_nega_feat, _) = model(audio_posi, visual_posi,visual_nega,question)
        out_qa = model(audio_posi, visual_posi,visual_nega,question)

        # loss_match_1 = criterion_contr(loc_audio_posi_feat, loc_visual_posi_feat, loc_visual_nega_feat)
        # loss_match_2 = criterion_contr(glo_audio_posi_feat, glo_visual_posi_feat, glo_visual_nega_feat)
        
        loss_qa = criterion_qa(out_qa, target)
        # loss = loss_qa + 0.1 * loss_match_1 + 0.1 * loss_match_2
        loss = loss_qa
...
def eval(model, val_loader, epoch):
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(val_loader):
            ...
            # (preds_qa, _, _, _, _, _, _, _) = model(audio_posi, visual_posi,visual_nega,question)
            preds_qa = model(audio_posi, visual_posi,visual_nega,question)
...
def test(model, test_loader):
    ...
    print('Audio Counting num:', sum(A_count))
    print('Audio Cmp num:', sum(A_cmp))
    print('Visual Counting num:', sum(V_count))
    print('Visual Loc num:', sum(V_loc))
    print('AV Ext num:', sum(AV_ext))
    print('AV counting num:', sum(AV_count))
    print('AV Loc num:', sum(AV_loc))
    print('AV Cmp num:', sum(AV_cmp))
    print('AV Temporal num:', sum(AV_temp))
