...
def train(args, model, train_loader, optimizer, criterion_qa, criterion_contr, epoch):
    ...
    for batch_idx, sample in enumerate(tqdm.tqdm(train_loader)):
        ...
        optimizer.zero_grad()
        (out_qa,audio_posi_feat,visual_posi_feat,visual_nega_feat,) = model(audio_posi, visual_posi,visual_nega,question)

        loss_match_1 = criterion_contr(audio_posi_feat, visual_posi_feat, visual_nega_feat)
...
def test(model, test_loader):
    ...
    total = 0
    correct = 0
    samples = json.load(open('/home/jovyan/Bert/data/json/avqa-test.json', 'r'))
    A_count = []
    A_cmp = []
    V_count = []
    V_loc = []
...
def main(args):
    ...
    if args.mode == 'train':
        ...
        for epoch in range(start_epoch, args.epoches + 1):
            ...
            train(args,model,train_loader,optimizer,criterion_qa,criterion_contr,epoch=epoch,)
            scheduler.step()
