...
class self_att(nn.Module):
    ...
    def forward(self, q_k_v):
        feat_att = self.self_att(q_k_v,q_k_v)[0].squeeze(dim=1)
...
class Global_Branch(nn.Module):
    def __init__(self, hops, hidden_dim, num_heads, dropout=0.1):
        ...
        self.attention_audio = Attention(self.hidden_dim * 2, n_head=num_heads, score_function=self.score_function, dropout=dropout)
        self.attention_video = Attention(self.hidden_dim * 2, n_head=num_heads, score_function=self.score_function, dropout=dropout)
        self.attention_audio2video = Attention(self.hidden_dim * 2, n_head=num_heads, score_function=self.score_function, dropout=dropout)
        self.attention_video2audio = Attention(self.hidden_dim * 2, n_head=num_heads, score_function=self.score_function, dropout=dropout)
    ...
    def forward(self, audio_memory, video_memory):
        ...
        for _ in range(self.hops):
            # audio branch
            it_al_audio2audio = self.attention_audio(audio_memory, et_audio)[0].squeeze(dim=1)
            it_al_video2audio = self.attention_video2audio(audio_memory, et_video)[0].squeeze(dim=1)
            it_al_audio = (it_al_audio2audio + it_al_video2audio) / 2
            # video branch
            it_al_video2video, att_map = self.attention_video(video_memory, et_video)
            it_al_video2video = it_al_video2video.squeeze(dim=1)
            it_al_audio2video = self.attention_audio2video(video_memory, et_audio)[0].squeeze(dim=1)
        ...
        return et_audio, et_video, att_map
...
class AVQA_Fusion_Net(nn.Module):
    ...
    def forward(self, audio_posi, video_posi, video_nega, question):
        ...
        loc_et_posi_audio, loc_et_posi_video, loc_att_map = self.Local_Branch(question_memory, audio_memory, video_posi_memory)
        # stage_1 co-attention branch of positive audio and negative video
        loc_et_nega_video, glo_att_map, _ = self.Local_Branch(question_memory, audio_memory, video_nega_memory)
        # print('loc_att_map: ', loc_att_map.shape)
        
        # global video feature
        # stage_1 co-attention branch of positive audio and positive video
        glo_et_posi_audio, glo_et_posi_video, glo_att_map = self.Global_Branch(audio_memory, video_posi_memory)
        _, glo_et_nega_video, _ = self.Global_Branch(audio_memory, video_nega_memory)
        ...
        audio_feat_att, visual_posi_feat_att, _ = self.Global_Local_Fusion(question_memory, local_feature, global_feature)
